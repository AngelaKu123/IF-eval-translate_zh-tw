...
def count_words(text):
  ...
  # -------
  if langdetect.detect(text) != ("en" or "sl"):  # .startswith("zh"):
    num_words = sum(1 for ch in text if '\u4e00' <= ch <= '\u9fff')
  else:
  # -------
...
def count_sentences(text):
  ...
  # -------
  if langdetect.detect(text) != ("en" or "sl"):
    SPLIT_TAG = "<split>"
    text = re.sub(r'(！？|？！|\?!|!\?)([^”’])', rf"\g<1>{SPLIT_TAG}\g<2>", text)
    text = re.sub(r'([。！？\?!])([^”’])', rf"\g<1>{SPLIT_TAG}\g<2>", text)
    text = re.sub(r'(\.{3,6})([^”’])', rf"\g<1>{SPLIT_TAG}\g<2>", text)  # ... or ......
    text = re.sub(r'(…{2})([^”’])', rf"\g<1>{SPLIT_TAG}\g<2>", text)  # "……", two \u2026
    text = re.sub(r'([。！？\?][”’])([^，。！？\?])', rf"\g<1>{SPLIT_TAG}\g<2>", text)
    sentences = [s for s in text.split(SPLIT_TAG) if s.strip()]
